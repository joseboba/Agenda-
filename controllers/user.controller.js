      ...
      if (err) {
        ...
        user.save((err, userSaved) => {
          if (err) {
            res
              .status(500)
              .send({ message: "Error en el servidor, intente más tarde" });
          } else {
            if (userSaved) {
    ...
    User.findOne(
      ...
      (err, userFound) => {
        if (err) {
          ...
          res
            .status(200)
            .send({ message: "Bienvenido a su cuenta: ", userFound });
        } else {
  ...
  User.findById(userId, (err, userFound) => {
    if (err) {
      ...
      if ((paramsContact.name, paramsContact.phone)) {
        ...
        res.status(200).send({
          message: "Ingrese los datos minimos para agregar un cotacto",
        });
      }
    } else {
        ...
        User.findOneAndUpdate(
          ...
          (err, contactUpdated) => {
            if (err) {
              ...
              res.status(200).send({
                message: "El contacto actualizado es: ",
                contactUpdated,
              });
            } else {
              res.status(404).send({ message: "El usuario no existe" });
